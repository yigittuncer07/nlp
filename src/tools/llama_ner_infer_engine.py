...
class BaseNERInferenceEngine():
    ...
    def infer_labels(self, tokens, max_length=256):
        """
        Perform inference on input tokens, remove special tokens, and merge subwords.
        Returns a list of labels, same lenght as the input list of tokens.
        """
        text = " ".join(tokens)
        ...
        # Filter for tokens starting with Ġ and the first subword token
        # Here we dont actually use the result tuple list, but I'm keeping it for degugging purposes
        results = []
        for i, token in enumerate(tokens):
            if offset_mapping[i] == [0, 0]:  # Ignore padding or special tokens
                continue

            # Add first token or tokens starting with Ġ
            if i == 1 or token.startswith("Ġ"):
                word = token.lstrip("Ġ")  # Remove Ġ from tokens
...
class UnllamaNerInferenceEngine(BaseNERInferenceEngine):
    def __init__(self, MODEL_PATH, id2label):
        super().__init__(MODEL_PATH, id2label, model_class=UnmaskingLlamaForTokenClassification)
